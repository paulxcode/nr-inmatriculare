...
img = cv2.imread(r'C:\Users\Paul\Desktop\pyA\img3.jpg')
...
contur = cv2.findContours(edged.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
print("A FOST DETECTAT UN NUMAR DE: ", detected)

mask = np.zeros(gray.shape, np.uint8)
new_image = cv2.drawContours(mask, [screenCnt], 0, 255, -1,)
new_image = cv2.bitwise_and(img, img, mask = mask)
cv2.imshow('Contor', new_image)
(x,y) = np.where(mask == 255)
(topx, topy) = (np.min(x), np.max(x))
